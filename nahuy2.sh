...
echo ""
wget -qO- --tries=0 "$url" | pv -s "$maxsize" | proot --link2symlink tar --preserve-permissions -Jxf - -C $folder || :
bin=start-$distro.sh
...
install_modded_os "https://github.com/profernick/Andronix-Modded-Os-Cracked/releases/download/manjaro_ubuntu/andronix_os.tar.xz"
else
install_modded_os "https://github.com/profernick/Andronix-Modded-Os-Cracked/releases/download/manjaro_ubuntu/andronix_os.tar.xz"
fi
fi
